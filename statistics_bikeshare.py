...
def map_input_to_days(days):
    if 'a' in days:
        days.remove('a')
        days.update(week_days.keys())
    if 'wdays' in days:
        days.remove('wdays')
        days.update(list(week_days.keys())[:5])
    if 'wends' in days:
        days.remove('wends')
        days.update(list(week_days.keys())[5:])
    return days
...
def get_filters():
    ...
    if not want_filter():
        ...
        cities = set(city_letters.values())
        months = set(months_num)
        days = week_days.values()
    else:
        ...
        while True:
            try:
                ...
                if 'a' in cities:
                    cities.remove('a')
                    cities.update(city_letters.keys())
                if all(e in city_letters.keys() for e in cities):
                    for val in cities:
                        tmp.add(city_letters[val])
                    cities = tmp
                    break
                else:
                    raise InvalidInput
        ...
        while True:
            try:
                ...
                if 'a' in months:
                    months.remove('a')
                    months.update(months_num)
                if all(e in months_num for e in months):
                    break
                else:
                    raise InvalidInput
            except InvalidInput:
        ...
        while True:
            try:
                ...
                days = map_input_to_days(days)

                tmp = set()
                if not days:
                    raise InvalidInput
                if all(e in week_days.keys() for e in days):
                    for val in days:
                        tmp.add(week_days[val])
                    days = tmp
                    break
                else:
                    raise InvalidInput
...
def load_data(cities, months, days):
    ...
    df = pd.concat((pd.read_csv(CITY_DATA[city]) for city in list(cities)), ignore_index=True)
...
def station_stats(df):
    ...
    if col_check(df, 'Start Station') and col_check(df, 'End Station'):
...
def user_stats(df):
    ...
    if col_check(df, 'Gender') and col_check(df, 'Trip Duration') and col_check(df, 'Month'):
